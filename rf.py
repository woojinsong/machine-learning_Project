...
clf = RandomForestClassifier(n_estimators=504, max_depth=19,random_state=0,n_jobs=-1)
...
#중요변수 추출
important_variable=[(column,variable) for column,variable in zip(df.columns,clf.feature_importances_)]
important_variable.sort(key=lambda x:x[1],reverse=True)#variable 중요도 순 정렬
for important in important_variable:
    print(important)
...
#랜덤 포레스트 파라미터 튜닝 
# params = { 'n_estimators' : list(range(500,600)),
